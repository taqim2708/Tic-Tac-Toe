...
class QLearningAgent:
    def __init__(self, letter, alpha=0.3, gamma=0.9, epsilon=0.1, q_table_file="qtable.pkl"):
        self.q = {}
        self.letter = letter
        self.alpha = alpha
        self.gamma = gamma
    ...
    for episode in range(n_episodes):
        ...
        while True:
            ...
            if game.current_winner == turn:
                ...
                agents['O' if turn == 'X' else 'X'].learn(state, action, -1, next_state, True)
                break
            elif game.is_draw():
                agent.learn(state, action, 0.5, next_state, True)
                agents['O' if turn == 'X' else 'X'].learn(state, action, 0.5, next_state, True)
                break
            else:
